...
public class DocumentService {
    ...
    public DocumentDto uploader(String emailUploadeur, Long projetId, Long etapeId, MultipartFile fichier) {
        ...
        Document documentEnregistre = documentRepository.save(document);

        notificationService.creer(projet.getEncadrant(), TypeNotification.DOCUMENT_DEPOSE,
                "Un document (\"" + nom + "\") a ete depose sur le projet \""
                        + projet.getSujet().getTitre() + "\".",
                "/projets/" + projet.getId() + "/documents");

        return DocumentDto.from(documentEnregistre);
    }
    ...
    /** Couverture du journal d'audit (etape 7.8) : suppression de document. */
    @Audite(action = "DOCUMENT_SUPPRIME", cible = "'Document#' + #documentId")
    @Transactional
     ...
     *
     * Couverture du journal d'audit (etape 7.8) : marquage archive.
     */
    @Audite(action = "DOCUMENT_ARCHIVE_STATUT_MODIFIE", cible = "'Document#' + #documentId",
            detail = "#archive ? 'Marque comme archive' : 'Demarque'")
    @Transactional
